...
const STATE_UPDATE_JSON_SCHEMA = `---META---
{
  ...
  "fullStateSnapshot": {
    "character": "<objeto completo del personaje ya reescrito>",
    "worldState": "<objeto completo del mundo con ingameDate actualizado>",
    "realisticAttributes": "<objeto completo de atributos>",
    "descriptors": "<objeto completo de descriptores>",
    "inventory": "<array completo>",
    "currency": "<objeto completo>",
    "npcs": "<array completo de NPCs conocidos>",
    "facciones": "<array completo de facciones>",
    "customSections": "<array completo de secciones personalizadas>",
    "consequenceQueue": "<array completo de consecuencias>",
    "personalHistory": "<array completo si se actualiza>"
  }
}`;
...
- "fullStateSnapshot": OBLIGATORIO en cada turno. Devuelve el estado completo resultante después de aplicar la narración, no solo deltas.
...
3. Devuelve fullStateSnapshot en todos los turnos con personaje, mundo, atributos, descriptores, inventario, NPCs, facciones, consecuencias y secciones personalizadas ya coherentes.
...
router.post("/generate", async (req, res) => {
  try {
    ...
    const narrationMessage = await anthropic.messages.create({
      model: MODEL,
      max_tokens: 4096,
      messages: [{ role: "user", content: narrationUserPrompt }],
      system: narrationSystemPrompt,
    });

    const narrationContent = narrationMessage.content[0];
    if (narrationContent.type !== "text") throw new Error("Unexpected narration response type");
    const narrative = narrationContent.text.trim();
    ...
    const stateMessage = await anthropic.messages.create({
      model: MODEL,
      max_tokens: 4096,
      messages: [{ role: "user", content: stateUserPrompt }],
      system: stateSystemPrompt,
    });

    const stateContent = stateMessage.content[0];
    if (stateContent.type !== "text") throw new Error("Unexpected state response type");

    // Parsear JSON del bloque de actualización de estado
    let meta: any = {};
    try {
      const jsonMatch = stateContent.text.match(/\{[\s\S]*\}/);
      if (jsonMatch) meta = JSON.parse(jsonMatch[0]);
    } catch {
      meta = {};
    }
...
router.post("/stream", async (req, res) => {
  try {
    ...
    if (stateContent.type === 'text') {
      const jsonMatch = stateContent.text.match(/\{[\s\S]*\}/);
      if (jsonMatch) metaJson = jsonMatch[0];
    }
...
{ "resumen": "<nuevo resumen completo aquí>" }`;
    ...
    try {
      ...
      resumen = content.text.slice(0, 500);
    }
